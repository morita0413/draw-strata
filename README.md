## 作成したプログラムについて
  - プログラムの機能
    - 定性表現(=記号列)から地層の断面図を描画するプログラム。主に3つの機能がある。
      1. 褶曲地層の描画。褶曲地層を表す定性表現から地層の断面図を描画する。描画時に妥当性(=対象とする定性表現かどうか)を判定する。また回転や反転などの操作も可能。
      2. 同一性、連続性の判定。2つの褶曲地層を表す定性表現を入力し、その2つが同一性を持つか、連続性を持つか調べる。
      3. 不整合地層の描画。不整合地層を表す定性表現から地層の断面図を描画する。
      <ol>
         <li>text</li>
         <li>text</li>
         <li>text</li>
